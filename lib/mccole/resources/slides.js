...
(function(doc) {
    ...
    // Add overall title to bottom left.
    function addTitle(slide, i, siteTitle, sectionTitle) {
        const title = newElement('span', 'full-title');
        if (i === 0) {
            title.innerText = siteTitle;
        } else {
            title.innerText = `${siteTitle} - ${sectionTitle}`;
        }
        slide.append(title);
    }
    ...
    function decorateSlides() {
        const siteTitle = doc.querySelector('meta[name="site-title"]').getAttribute('content');
        const sectionTitle = doc.querySelector('meta[name="section-title"]').getAttribute('content');
        const slides = doc.querySelectorAll('div.slide');
        const numSlides = slides.length;
        slides.forEach((s, i) => {
            addPageNumber(s, i, numSlides);
            addTitle(s, i, siteTitle, sectionTitle);
            applyDirectives(s);
        });
        slides[0].querySelector('h1').classList.add('title-page');
    }
